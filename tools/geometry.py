...
Boards are labelled A, B, C, ... by the MAC->label map below, which records where
each unit physically sits. Light one board (or a pair) to work out which physical
unit is which, measure the distances by hand, then solve for coordinates.
...
def labelled(found):
    """Label -> (MAC, port), using the physical placement recorded in LABEL.

    Sorted-MAC order was used originally, but a label has to name a *position* in
    the room, not a board: the plotting tools key their geometry off these letters,
    so moving a board between setups has to change its letter. Anything not in the
    map falls back to sorted order after the known boards, so an unrecognised unit
    still gets a handle instead of vanishing.
    """
    out = {label: (mac, found[mac]) for mac, label in LABEL.items() if mac in found}
    used = set(out)
    spare = (c for c in map(chr, range(ord('A'), ord('Z') + 1)) if c not in used)
    for mac in sorted(set(found) - set(LABEL)):
        out[next(spare)] = (mac, found[mac])
    return out
...
def solve_layout(labels, dist, bounds=None):
    """Classical MDS for a starting guess, then least-squares refine in 2D.

    `bounds` holds pairs that could not be measured exactly, as (op, value) with
    op in '<>'. A tape measure that stops at 5 m still says something useful about
    a longer pair, and for a 4-board array that one-sided fact is often exactly
    what picks between the two mirror-image layouts the exact distances allow.
    """
    bounds = bounds or {}
    n = len(labels)
    ...
    for (a, b), (_op, d) in bounds.items():       # the bound doubles as a guess
        D[idx[a], idx[b]] = D[idx[b], idx[a]] = d
    ...
    def resid(flat):
        X = flat.reshape(n, 2)
        out = [np.linalg.norm(X[idx[a]] - X[idx[b]]) - d for (a, b), d in dist.items()]
        for (a, b), (op, d) in bounds.items():
            r = float(np.linalg.norm(X[idx[a]] - X[idx[b]]))
            # one-sided: no cost anywhere inside the bound, linear outside it
            out.append(min(r - d, 0.0) if op == '>' else max(r - d, 0.0))
        return out

    # A bound is a non-convex constraint, so a single start can settle into the
    # mirror branch it forbids. Start from the MDS guess and from its reflection
    # and keep whichever converges lower.
    best = None
    for start in (X0, X0 * [1, -1]):
        out = least_squares(resid, start.ravel())
        if best is None or out.cost < best.cost:
            best = out
    X = best.x.reshape(n, 2)
...
def do_solve(args):
    ...
    for item in args.distances:
        op = next((c for c in '=<>' if c in item), None)
        if op is None:
            print(f'bad argument "{item}", expected e.g. AB=1.20 or AD>5.00')
            sys.exit(1)
        pair, val = item.split(op, 1)
        ...
        if op == '=':
            ...
            bounds[(pair[0], pair[1])] = (op, float(val))

    labels = sorted({l for p in list(dist) + list(bounds) for l in p})
    n = len(labels)
    need = n * (n - 1) // 2
    if len(dist) + len(bounds) != need:
        print(f'have {len(dist) + len(bounds)} distances for {n} boards, need {need}: '
              f'{" ".join("".join(p) for p in itertools.combinations(labels, 2))}')
        sys.exit(1)

    # A planar layout has 2n-3 degrees of freedom. Exact distances below that
    # leave the shape genuinely undetermined; exactly that many pin it down but
    # leave nothing over to cross-check, which changes how the residuals read.
    dof = 2 * n - 3
    if len(dist) < dof:
        print(f'WARNING: {len(dist)} exact distances for {dof} degrees of freedom -- '
              'the layout is underdetermined and the coordinates below are one of\n'
              '         many that fit. Measure another pair exactly.\n')

    X, eig, resid = solve_layout(labels, dist, bounds)
    ...
    if len(dist) == dof:
        print(f'  ({len(dist)} exact distances for {dof} degrees of freedom: the fit is exact by')
        print('   construction, so near-zero residuals confirm nothing. Any single bad')
        print('   measurement is absorbed silently into the coordinates.)')

    if bounds:
        print('\nUnmeasured pairs, predicted from the rest:')
        for (a, b), (op, d) in sorted(bounds.items()):
            r = float(np.linalg.norm(pos[a] - pos[b]))
            ok = r > d if op == '>' else r < d
            print(f'  {a}{b}: {r:6.3f} m  (you said {op}{d:.3f}) '
                  f'{"consistent" if ok else "<-- CONTRADICTS the bound"}')
        print('  Measuring one of these would turn the exact fit into a real check.')

    # Third eigenvalue of the MDS Gram matrix measures out-of-plane extent: it is
    # zero for a genuinely planar, self-consistent set of distances. A bounded pair
    # enters the MDS matrix as a guess, which would make that eigenvalue fiction.
    if n >= 4 and not bounds:
