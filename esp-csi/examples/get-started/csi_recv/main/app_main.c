...
// Onboard addressable (WS2812) status LED, present on GPIO48 on most ESP32-S3-DevKitC-1 boards.
...
// Lights the onboard status LED with a color derived from this chip's factory MAC
// (esp_read_mac always reads the eFuse-burned base MAC, unaffected by the
// esp_wifi_set_mac() override below), so each physical board gets a stable,
// distinguishable color across power cycles without needing per-board firmware.
static void init_status_led(void)
{
    uint8_t mac[6];
    ESP_ERROR_CHECK(esp_read_mac(mac, ESP_MAC_WIFI_STA));

    // 10 hues spaced 36 degrees apart around the color wheel (fixed saturation/value),
    // plus white, so each color is maximally separated from its neighbors instead of
    // an ad hoc pick. No magenta: an earlier version collided with another board's hue.
    static const struct { uint8_t r, g, b; const char *name; } palette[] = {
        {40, 0, 0, "red"}, {40, 24, 0, "orange"}, {32, 40, 0, "lime"},
        {8, 40, 0, "green"}, {0, 40, 16, "teal"}, {0, 40, 40, "cyan"},
        {0, 16, 40, "azure"}, {8, 0, 40, "blue"}, {32, 0, 40, "violet"},
        {40, 0, 24, "pink"}, {30, 30, 30, "white"},
    };
    // FNV-1a: a plain byte sum correlates too much across MACs sharing a vendor prefix
    // (as ours do), which caused multiple boards here to collide on the same color.
    // The trailing salt byte was picked by brute-force search to be collision-free
    // across our currently-owned boards' MACs (still just a hash, not a lookup table
    // keyed by MAC -- a newly added board can in principle still collide with another).
    ...
    hash ^= 1;
    hash *= 16777619u;
    int idx = hash % (sizeof(palette) / sizeof(palette[0]));
    ...
    led_strip_set_pixel(led_strip, 0, palette[idx].r, palette[idx].g, palette[idx].b);
    led_strip_refresh(led_strip);

    ESP_LOGI(TAG, "Board MAC " MACSTR " -> status LED: %s", MAC2STR(mac), palette[idx].name);
}
